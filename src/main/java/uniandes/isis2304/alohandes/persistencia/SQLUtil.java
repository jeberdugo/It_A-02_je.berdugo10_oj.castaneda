 ...
 * concepto BAR de AlohAndes Nótese que es una clase que es sólo conocida en el
...
class SQLUtil {
	...
	public long[] limpiarAlohAndes(PersistenceManager pm) {
		Query qGustan = pm.newQuery(SQL, "DELETE FROM " + pp.darTablaUsuario());
		Query qSirven = pm.newQuery(SQL, "DELETE FROM " + pp.darTablaOperador());
		Query qVisitan = pm.newQuery(SQL, "DELETE FROM " + pp.darTablaCliente());
		Query qBebida = pm.newQuery(SQL, "DELETE FROM " + pp.darTablaReserva());
		Query qTipoBebida = pm.newQuery(SQL, "DELETE FROM " + pp.darTablaOferta());
		Query qBebedor = pm.newQuery(SQL, "DELETE FROM " + pp.darTablaAlojamiento());
